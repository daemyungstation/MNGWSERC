...
public class OMBOrderMngController extends EmfController {
	...
	{		
		...
		try
		{
			...
			List<String> fixedAbleIdList = Arrays.asList( "lgorder",  "csvorder", "mmorder", "dyorder", "dhorder", "isollat", "isorder", "sonoseason", "wmnetwork");
			List<String> idList = lgnMap.getList("idList");
			boolean notSupportedId = false;
			for(String idInList : idList){
				if( !fixedAbleIdList.contains(idInList) ){
					notSupportedId = true;
				}
			}
			if( !fixedAbleIdList.contains(id) ){
				notSupportedId = true;
			}
