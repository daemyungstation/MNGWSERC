 ...
 * </pre>
 */
@Service("oMBOrderMngService")
public class OMBOrderMngServiceImpl extends EmfAbstractService implements OMBOrderMngService {

	private static final Logger LOG = Logger.getLogger(OMBOrderMngServiceImpl.class.getName());
	...
	public EmfMap selectOrderMngList(EmfMap emfMap) throws Exception
	{
		...
		emfMap.put("id", id);
		if( emfMap.get("idList") instanceof String) {
			emfMap.put("idList", new String[] { (String)emfMap.get("idList") });
		}
		if(!"".equals(id) || (emfMap.get("idList") != null && ((String[])emfMap.get("idList")).length > 0 ) )
		{
	...
	public void updateOrderMng(EmfMap emfMap) throws Exception
	{
		...
		if("ord".equals(emfMap.getString("type")))
		{
			...
			else
			{
				...
				List<String> admIdList = emfMap.getList("admId");
				List<String> accntNoList = emfMap.getList("accntNo");
				if( admIdList != null && admIdList.size() > 0 && accntNoList != null && accntNoList.size() > 0 ) {
					for( int i = 0; i < accntNoList.size() ; i++) {
						String accountNo = accntNoList.get(i);
						List<String> subAccntNoList = Arrays.asList(accountNo);
						for( int j =0; j < admIdList.size(); j++) {
							String admId = admIdList.get(j);
							if( admId.startsWith(accountNo) ) {
								String admIdValue = admId.split("_")[1];
								id = admIdValue;
								break;
							}
						}
						LOG.info(String.format(" 발주정보 : %s, %s, %s", emfMap, id, accountNo));
						insertDeliveryInfoByAccount(emfMap, id, subAccntNoList);
					}
				} else {
					insertDeliveryInfoByAccount(emfMap, id, accntNoList);
				}
				/*
				 홈페이지 개인정보 로그 - 보안관련 로직 신규 구현
				 2017.12.26
...
//			logMap.put("id", RequestContextHolder.getRequestAttributes().getAttribute("id", RequestAttributes.SCOPE_SESSION));
	...
	private void insertDeliveryInfoByAccount(EmfMap emfMap, String id, List<String> accntNoList) throws Exception {
		emfMap.put("id", id);
		emfMap.put("b2bStat", "");
		...
		if("csvorder".equals(id) || "mmorder".equals(id) || "dyorder".equals(id) || "lgorder".equals(id) || "dhorder".equals(id) || "isollat".equals(id)  || "isorder".equals(id) || "sonoseason".equals(id) || "wmnetwork".equals(id))
		{
			if(accntNoList.size() > 0)
			{
				emfMap.put("accntNo", accntNoList);
		...
		{
			emfMap.put("errorMsg", "발주관리 아이디 값이 없습니다. : " + id);
		}
	}
