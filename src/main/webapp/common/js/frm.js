...
*/
 ...
 //허용 파일
var extfile = Array("pdf","jpg","gif","png","ppt","xls","doc","pptx","xlsx","docx","hwp","txt", "bmp");
...
function validateEx(frm, cfrmMsg) 
{
    ...
    if (validateExCreate == false) 
    {
        ...
        var html  = '<iframe id="' + targetName + '" name="' + targetName + '" height="'+height+'px" ' + 'width="100%" frameborder="0" marginwidth="0" marginheight="0" ' + 'scrolling="no"';
        	html += '></iframe>';
...
FrmChk.prototype.number = function() 
{
    ...
    {
        if (this.obj.Rvalue.length != this.chkAttVal)
        {
        	return false;
        }
    }
...
FrmChk.prototype.passChk = function() {
	...
	if(pws != "" || pws.length != 0)
	{
		...
		if (!this.isAlphaAndNumberAndSpecial(pws))	
		{
			this.addErrMsg = "비밀번호는 영문과 숫자 그리고 특수문자의 혼합 문자만 사용 가능합니다.";
			return false;
		}

		if (pws.length < 8) 
		{
			this.addErrMsg = "8자 미만 문자열은 비밀번호로 사용하실 수 없습니다";
			return false;
		}

		if (pws.length > 20)	
		{
			this.addErrMsg = "20자 초과 문자열은 비밀번호로 사용하실 수 없습니다";
			return false;
		}
	}
...
FrmChk.prototype.isAlphaAndNumberAndSpecial = function(val)
{
	val = val.toLowerCase();
	
	var num = val.search(/[0-9]/g);
	var eng = val.search(/[a-z]/ig);
	var spe = val.search(/[\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi);
