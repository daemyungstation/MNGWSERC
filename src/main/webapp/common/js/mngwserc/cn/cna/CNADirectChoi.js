...
$(function() {
	...
	$("#popup").on("hidden.bs.modal", function() {
		...
		$(".td_name input:button").addClass("hidden");
...
function inserPopOpen() {
	...
	$("#pop_Label").text("다이렉트 초이스 이미지 등록");
	$("#write_btn").text("등록");
 ...
 *  리스트 > 상세보기
 */
function selectDirectChoiDtl(seq) {
	$.ajax({
		url : "./view.ajax",
		type : "post",
		async : false,
		...
		success : function(res) {
			console.log(res)
			$("#pop_Label").text("다이렉트 초이스 이미지 수정");
			$("#write_btn").text("수정");
			$("form[name='frm_write']").attr("data-type", "update");
			$("input:hidden[name='seq']").val(seq);
			
			var dirctInfo = res.data.dirctInfo;
			var imgFileList = res.data.imgFileList;
			
			// 값세팅
			$("input:text[name='title']").val(dirctInfo.title);
			$("input:radio[name='use_yn']:radio[value='"+ dirctInfo.useYn +"']").prop("checked", true);
			$("input:radio[name='use_yn']").closest("td").attr("data-useyn", dirctInfo.useYn);
			...
			for(var i=0; i<pop_img.length; i++) {
				popTable.find(".td_name span").eq(i).text(pop_img[i].fileNm);
				popTable.find(".td_name input:button").eq(i).removeClass("hidden");
				popTable.find(".td_name input:button").eq(i).attr("data-fileseq", pop_img[i].fileSeq);
				popTable.find(".td_name").closest("tr").eq(i).find("td:eq(2)").text(pop_img[i].fileSize);
				popTable.find(".td_name").closest("tr").eq(i).find("td:eq(3)").text(pop_img[i].fileType);
				popTable.find(".td_name").closest("tr").eq(i).find("td:eq(4) button").removeClass("hidden");
			}
...
function writeBtn() {
	...
	for(var i=0; i<$_loop.length; i++) {
		...
		if(popShow_chk) {
			var pc_img = $_imgAdd.find(".td_name span").eq(2).text();
			var mobile_img = $_imgAdd.find(".td_name span").eq(3).text();
		...
		$_imgAdd.find("input:file:eq(2)").attr("name", "pc_pop_" + cnt);
		$_imgAdd.find("input:file:eq(3)").attr("name", "mobile_pop_" + cnt);
	}// end for
	...
	if(form_type === 'update') {	// 수정 팝업
		if(db_useYn === 'Y') {	// 현재 db에 저장되어 있는 노출여부 Y
			...
			if(cur_useYn === 'Y') {
				if(useYnChk === '1') {
					msg = "다이렉트 초이스 이미지가 노출중입니다. 이미지를 수정하시겠습니까?";
				} else {
					msg = "수정하시겠습니까?";
				}
			} else {
		...
		if(cur_useYn === 'Y') {	// 노출 여부 Y로 등록시
			if(useYnChk === '1') {
				console.log("??")
				msg = "다이렉트 초이스 이미지가 노출중입니다. 새로 등록하시겠습니까?";
			} else {
				msg = "등록하시겠습니까?";
			}
		} else { // 노출 여부 N으로 등록시
...
function preview(e) {
	...
	if ($this.closest("tr").find("input:file").val() != '') {
		...
		$("#preview").find("img").attr("src", "/mngwserc/cna/direct/previewImg.do?fileSeq=" + fileseq);
		$("#preview").show();
	}
}
...
function loopHTML() {
	$.ajax({
		url : "/common/template/CNADirectChoi_LOOP.jsp",
		type : "post",
		dataType : "html",
		async : false,
		success : function(res) {
