...
<!-- 
	######################################################################
	파일명 		:	SMCDnldFormList.jsp
	프로그램 명 : 	다운로드 양식 목록을 조회한다.
	설명		: 	다운로드 양식 목록을 조회하는 페이지
	작성자		: 	허진영
	작성일		:	2016.03.30
	수정일자	 			수정자				수정내용
	=====================================================================
...
<form name="frm" method="post" action="${pageLink}">
	...
	<div class="well well-small">
		<select name="f">
			<option value="1" <c:if test="${rtnMap.f eq '1'}">selected</c:if>>양식명</option>
		</select>
	...
	<table class="table table-bordered table-hover">
		...
		<tbody>
			...
			<c:if test="${fn:length(rtnMap.list) eq 0}">
				<tr>
					<td class="lt_text3" colspan="5" style="text-align:center">
			...
			<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
				<tr>
					<td style="text-align:center;">
						<input type="checkbox" name="delSeq" value="${list.dnldFormSeq}" />
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectDnldFormDtl('${list.dnldFormSeq}');">
						${list.totCnt - (rtnMap.pageIndex - 1) * rtnMap.paginationInfo.pageSize - status.count + 1}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectDnldFormDtl('${list.dnldFormSeq}');">
						${egov:tldCutString(list.dnldFormNm, '...', '35')}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectDnldFormDtl('${list.dnldFormSeq}');">
						${list.regId}
					</td>
...
<div style="float:left;">
	<a href="javascript:deleteDnldFormList();" class="btn btn-danger">삭제</a>
</div>
...
<script type="text/javascript">
	...
	//상세 가져오기
	function selectDnldFormDtl(dnldFormSeq)
	{
		var f = document.frm;
		
		f.action = "./write.do";
		f.dnldFormSeq.value = dnldFormSeq;
		f.submit();
	}
	
	//선택 삭제하기
	function deleteDnldFormList()
	{
