...
<form name="frm" method="post" action="${pageLink}">
	...
	<div class="well well-small">
		...
		<select name="ptupYn">
			...
			<option value="Y" <c:if test="${rtnMap.alwaysYn eq 'Y'}">selected</c:if>>Y</option>
			<option value="N" <c:if test="${rtnMap.alwaysYn eq 'N'}">selected</c:if>>N</option>
		</select>
		&nbsp;&nbsp;
		<select name="f">
			...
			<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
				<tr>
					<td style="text-align:center;">
						<input type="checkbox" name="delSeq" value="${list.idx}" />
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectPopDtl('${list.idx}');">
						${list.totCnt - (rtnMap.pageIndex - 1) * rtnMap.paginationInfo.pageSize - status.count + 1}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectPopDtl('${list.idx}');">
						${egov:tldCutString(list.name, '...', '35')}
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectPopDtl('${list.idx}');">
						<c:choose>
							<c:when test="${list.alwaysYn eq 'Y'}">
								상시
							</c:when>
							<c:otherwise>
