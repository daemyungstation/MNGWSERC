			...
			<tr>
				...
				<th width="52%">
					<c:choose>
						...
						<c:when test="${rtnMap.oprtGuideGb eq 'trms'}">
							이용약관 
						</c:when>
						<c:when test="${rtnMap.oprtGuideGb eq 'iptnt'}">
							중요정보 고시사항 
						</c:when>
						<c:when test="${rtnMap.oprtGuideGb eq 'otsc'}">
							외주용역 안내 
						</c:when>
						<c:when test="${rtnMap.oprtGuideGb eq 'legal'}">
							법적고지
						</c:when>
