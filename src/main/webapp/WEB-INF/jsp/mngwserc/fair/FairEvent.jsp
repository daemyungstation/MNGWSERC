...
<h6 style="float:left;">대명아임레디 박람회 - 이벤트 관리 : 총 ${rtnMap.list[0].totCnt}건</h6>
...
<form name="frm" action ="${pageLink}">
	...
	<table class="table table-bordered table-hover sorttable">
		<caption style="display: none;">대명아임레디 박람회 - 이벤트 관리</caption>
	 	<thead>
	  		<tr>
			    <th class="td-no">번호</th>
			    <th class="td-auto">이미지</th>
			    <th class="td-title">링크</th>
	...
	<ul id="list_sort">
		<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
	  		<li class="ui-state-default" data-seq="${ list.feSeq }" data-order="${ list.feOrder }">
     			<table class="list_sort_table table table-bordered sorttable">
     				<tr>
					    ...
					    <td class="td-auto" style="text-align:left;">
					    	<c:if test="${ list.feImage1WSaveNm ne null }">
					    	<input type="button" value="PC-[${ list.feImage1WRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.feImage1WPath }" data-save="${ list.feImage1WSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.feImage1MSaveNm ne null }">
					    	<input type="button" value="MO-[${ list.feImage1MRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.feImage1MPath }" data-save="${ list.feImage1MSaveNm }"/>
					    	</c:if>
					    </td>
					    <td class="td-title">${ list.feLink }</td>
					    <td class="td-time" style="text-align:center;">
					    	${ list.feStime } ~ ${ list.feEtime }
					    </td>
					    <td class="td-use" style="text-align:center;">
					    	<c:if test="${list.feStatus eq 'Y'}">사용</c:if>	
					    	<c:if test="${list.feStatus eq 'N'}">미사용</c:if>	
					    	<c:if test="${list.feStatus eq 'A'}">상시사용</c:if>	
					    </td>
					    <td class="td-order" style="text-align:center;">
					    	${list.feOrder}
					    </td>
					    <td class="td-mng" style="text-align:center;">
					    	<a href="./write.do?seq=${list.feSeq}" class="btn btn-warning btn-xs">
					    		수정
					    	</a>
					    	<a href="javascript:del('${list.feSeq}');" class="btn btn-danger btn-xs">
					    		삭제
					    	</a>
				    	</td>
				  	</tr>
