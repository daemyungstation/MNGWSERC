...
<h6 style="float:left;">대명아임레디 박람회 - 제품 상세 관리 : 총 ${rtnMap.list[0].totCnt}건</h6>
...
<form name="frm" action ="${pageLink}">
	...
	<table class="table table-bordered table-hover sorttable">
		<caption style="display: none;">대명아임레디 박람회 - 제품 상세 관리</caption>
	 	<thead>
	  		<tr>
			    <th class="td-no">번호</th>
			    <th class="td-title">제목</th>
			    <th class="td-title">서브제목</th>
			    <th class="td-title">설명</th>
	...
	<ul id="list_sort">
		<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
	  		<li class="ui-state-default" data-seq="${ list.fpSeq }" data-order="${ list.fpOrder }">
     			<table class="list_sort_table table table-bordered sorttable">
     				<tr>
					    <td class="td-no" style="text-align:center;">
					   	...
					   	<td class="td-title">${ list.fpMainTitle }</td>
					    <td class="td-title">${ list.fpMainSubtitle }</td>
					    <td class="td-title">${ list.fpMainDesc }</td>
					    <td class="td-auto" style="text-align:left;">
					    	<c:if test="${ list.fpMainImageBgSaveNm ne null }">
					    	<input type="button" value="BG-[${ list.fpMainImageBgRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fpMainImageBgPath }" data-save="${ list.fpMainImageBgSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.fpMainImageWSaveNm ne null }">
					    	<input type="button" value="PC-[${ list.fpMainImageWRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fpMainImageWPath }" data-save="${ list.fpMainImageWSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.fpMainImageMSaveNm ne null }">
					    	<input type="button" value="MO-[${ list.fpMainImageMRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fpMainImageMPath }" data-save="${ list.fpMainImageMSaveNm }"/>
					    	</c:if>
					    </td>
					    <td class="td-time" style="text-align:center;">
					    	${ list.fpStime } ~ ${ list.fpEtime }
					    </td>
					    <td class="td-use" style="text-align:center;">
					    	<c:if test="${list.fpStatus eq 'Y'}">사용</c:if>	
					    	<c:if test="${list.fpStatus eq 'N'}">미사용</c:if>	
					    	<c:if test="${list.fpStatus eq 'A'}">상시사용</c:if>	
					    </td>
					    <td class="td-order" style="text-align:center;">
					    	${list.fpOrder}
					    </td>
					    <td class="td-mng" style="text-align:center;">
					    	<a class="btn btn-success btn-xs" onclick="javascript:productCopy(${list.fpSeq});">
					    		복사
					    	</a>
					    	<a href="./write.do?seq=${list.fpSeq}" class="btn btn-warning btn-xs">
					    		수정
					    	</a>
					    	<a href="javascript:del('${list.fpSeq}');" class="btn btn-danger btn-xs">
					    		삭제
					    	</a>
				    	</td>
				  	</tr>
		...
		if(confirm("해당 상품을 복사하시겠습니까?"))
		{
			jQuery.ajax({
				...
				success: function(data) {
					document.location.href = './write.do?seq='+ data.seq;
				}
			});
		}
	}
