...
.table-bordered #shUrl .txt_url_input{
	width:8%;
}
.table-bordered #shUrl #urlText{
	width:100%;
}
</style>
<div class="wrap_pop">
	...
	<table class="table table-bordered">
		...
		<tbody>
			...
			<tr>
				...
				<td class="_banner_img_td">
					<div class="title">
						...
						<span class="tit"></span>
						<span class="txt">*PC 1920x600, Mobile 640x350의 JPG, PNG, GIF 파일을 등록해주세요.</span>
					</div>					
						...
						<tr>
							<td>PC &nbsp;
								<span class="btn fileinput-button">
				                    ...
				                    <span>파일추가</span>
				                    <input type="file" name="ppImgBtn" accept=".gif, .jpg, .png"/>
				                </span>
							</td>
							<td class="td_name">
								<span></span>
						...
						<tr>
							<td>Mobile &nbsp;
								<span class="btn fileinput-button">
				                    <i class="icon-upload"></i>
				                    <span>파일추가</span>
				                    <input type="file" name="pmImgBtn" accept=".gif, .jpg, .png"/>
				                </span>
							</td>
							<td class="td_name">
								<span></span>
			...
			<tr>
				...
				<td>
					<b>경로 선택</b> &nbsp;
					<input type="radio" name="linkGubun" value="URL">&nbsp;url&nbsp;
					<input type="radio" name="linkGubun" value="ZERO">&nbsp;제로 초이스&nbsp;
					<input type="radio" name="linkGubun" value="DIRECT">&nbsp;다이렉트 초이스&nbsp;
					<input type="radio" name="linkGubun" value="LIMIT">&nbsp;리미티드 초이스&nbsp;
					<br/>
					<div id="shUrl" style="display:none;">
						<span class="txt_url_input">url 입력</span>
						<input type="text" class="inputType" name="urlText" id="urlText">
					</div>
					* URL을 입력하거나 상품을 선택하세요.(복수 선택불가)
				</td>
			</tr>
			<tr>
				<th>노출여부</th>
...
// 		var fileseq = $(this).data("fileseq");
	...
	$("#writePrdMgrBtn").click(function(e) {
		...
		if($("input[name=linkGubun]").val() == "url"){
			if($("input[name=urlText]").val() == ""){
				alert("URL을 입력해 주세요.");
				$("input[name=urlText]").focus();
				return;
			}
		}
...
$('input[name=linkGubun]').change(function(){
	if($(":input:radio[name=linkGubun]:checked").val() == 'URL'){
		$("#urlText").attr('readonly', false);
		$("#urlText").val('');
	} else {
		if($(":input:radio[name=linkGubun]:checked").val() == 'ZERO'){
			$("#urlText").val('/zeroIndex');
		} else if ($(":input:radio[name=linkGubun]:checked").val() == 'DIRECT'){
			$("#urlText").val('/directIndex');
		} else if ($(":input:radio[name=linkGubun]:checked").val() == 'LIMIT'){
			$("#urlText").val('/limitIndex');
		}
		$("#urlText").attr('readonly', true);
	}
});
...
function imgHTML(status, imgList) {
	...
	if (imgList != null) {
		for (var i = 0; i < imgList.length; i++) {
			$("._banner_img_td .td_name").eq(i).find("span").text(imgList[i].realFileNm);
			$("._banner_img_td .td_name").eq(i).find(".btn_pop_preview").removeClass("hidden");
			$("._banner_img_td .td_name").eq(i).find(".btn_pop_preview").show();
			$("._banner_img_td .td_name").eq(i).find(".btn_pop_preview").attr("data-fileSeq",imgList[i].fileSeq);
			$("._banner_img_td .td_name").eq(i).siblings().eq(1).text(imgList[i].fileSize);
			$("._banner_img_td .td_name").eq(i).siblings().eq(2).text(imgList[i].fileExtn);
			$("._banner_img_td .td_name").eq(i).siblings().eq(3).find("button").show();
			
			$("input[name=urlText]").val(imgList[i].url);
		}
	}
