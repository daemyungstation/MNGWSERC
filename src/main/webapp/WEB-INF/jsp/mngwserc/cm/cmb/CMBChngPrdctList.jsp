...
<!-- 
	...
	프로그램 명 : 	전환서비스 상품 목록을 조회한다.
	설명		: 	전환서비스 상품 목록을 조회하는 페이지
	작성자		: 	김대환
	작성일		:	2016.02.19
	수정일자	 			수정자				수정내용
	=====================================================================
	...
	<table class="table table-bordered table-hover">
		...
		<thead>
			<tr>
				...
				<th width="6%">번호</th>
				<th width="10%">서비스 구분</th>
				<th width="55%">서비스 내용</th>
				<th width="13%">작성자</th>
				<th width="11%">작성일</th>
			</tr>
		</thead>
		<tbody>
			...
			<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
				<tr>
					...
					<td style="text-align:center; cursor:pointer;" onclick="selectChngPrdctDtl('${list.prdctSeq}');">
						${list.totCnt - (rtnMap.pageIndex - 1) * rtnMap.paginationInfo.pageSize - status.count + 1}
					</td>					
					<td style="text-align:center; cursor:pointer;" onclick="selectChngPrdctDtl('${list.prdctSeq}');">
						${list.prdctGb}							
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectChngPrdctDtl('${list.prdctSeq}');">
						${list.prdctNm}
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectChngPrdctDtl('${list.prdctSeq}');">
						${list.regId}
...
<div style="float:left;">
	<a href="javascript:deleteChngPrdctList();" class="btn btn-danger">삭제</a>
	<a href="javascript:insertChngPrdctCopy();" class="btn btn-info">복사</a>
</div>
...
<script type="text/javascript">
	...
	//상세 가져오기
	function selectChngPrdctDtl(prdctSeq)
	{
		var f = document.frm;	
		
		f.action = "./view.do";
		f.prdctSeq.value = prdctSeq;
		f.submit();
	}
	
	//선택 삭제하기
	function deleteChngPrdctList()
	{
