    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/provider/lib/seditor/popup/quick_photo/FileUploader_html5_r.asp'; 	//upload URL
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		...
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/mngwserc/editorimageupload/action.do',	//샘플 URL입니다.
 	        sCallback : '/egov/seditor/popup/quick_photo/PhotoCallback.jsp',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			oFileInfo[0].FileDescription = unescape(oFileInfo[0].FileDescription);
			
			var img = '<img src="/editor'+oFileInfo[0].sFileName+'" alt="'+oFileInfo[0].FileDescription+'" />';
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [img]);
			//본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
			//e(oFileInfo[0])
			//var img = '<img src="'+oFileInfo[0].sFileURL+'" title="'+oFileInfo[0].sFileName+'" alt="'+oFileInfo[0].FileDescription+'" />';
			//e(img)
			//opener.oEditors.getById["PP_BODY"].exec("PASTE_HTML", [img]);
			//opener.nhn.husky.PopUpManager.setCallback(window, "PASTE_HTML", img);
		}
	}
