...
for (var i = 0; i < sSrc.length; i++) 
{
	...
	if (s.match(/CODUserMenu.js?/) != null) 
	{
        var qsE = s.split("/common/js/mngwserc/co/cod/CODUserMenu.js");
...
jQuery(function () {
	jQuery("#divCategoris")	
			...
			"types" : {
				...
				"types" : {
					...
					"drive" : {
						...
						"valid_children" : [ "default", "folder", "cms", "etc"],
						"icon" : {
		...
		jQuery("#mmenu a").click(function (event) {
			...
			{
				case "add_root":
					jQuery("#divCategoris").jstree("create", -1, "last", { "attr" : { "rel" : "etc" } }, false, false);					
					break;
				case "add_folder":
					jQuery("#divCategoris").jstree("create", null, "last", { "attr" : { "rel" : "etc" } });
					break;
				case "remove":
			...
			jQuery.post("/mngwserc/cod/adm/updateMenuInf.ajax",
				{
					"menuSeq": id,
					"menuGb": menuGb,
					"userUseYn": jQuery("#userUseYn").val(),
					"userWndYn" : jQuery("#userWndYn").val(),
					"userLink": admLink.replace("/mngwserc", ""),
					"admLink" : admLink,
					"seo": jQuery("#seo").val(),
...
//현재 배포중인거 있는지 찾기
function chkStatus(menuSeq)
{
	jQuery.get("/mngwserc/cod/user/chkStatus.ajax",
		{
			"menuSeq" : menuSeq
		},
		function(r) {
			...
			if(rtnMap)
			{
				jQuery("input[name='refSeq']").val(menuSeq);
