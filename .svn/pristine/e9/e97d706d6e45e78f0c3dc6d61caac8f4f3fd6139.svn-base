...
<h6 style="float:left;">대명아임레디 박람회 - 탑배너 관리 : 총 ${rtnMap.list[0].totCnt}건</h6>
...
<form name="frm" action ="${pageLink}">
	...
	<ul id="list_sort">
		<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
	  		<li class="ui-state-default" data-seq="${ list.fbtSeq }" data-order="${ list.fbtOrder }">
     			<table class="list_sort_table table table-bordered sorttable">
     				<tr>
					    ...
					    <td class="td-auto" style="text-align:left;">
					    	<c:if test="${ list.fbtImageBgSaveNm ne null }">
					    	<input type="button" value="배경-[${ list.fbtImageBgRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fbtImageBgPath }" data-save="${ list.fbtImageBgSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.fbtImageWSaveNm ne null }">
					    	<input type="button" value="PC-[${ list.fbtImageWRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fbtImageWPath }" data-save="${ list.fbtImageWSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.fbtImageMSaveNm ne null }">
					    	<input type="button" value="MO-[${ list.fbtImageMRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fbtImageMPath }" data-save="${ list.fbtImageMSaveNm }"/>
					    	</c:if>
					    </td>
					    <td class="td-time" style="text-align:center;">
					    	${ list.fbtStime } ~ ${ list.fbtEtime }
					    </td>
					    <td class="td-use" style="text-align:center;">
					    	<c:if test="${list.fbtStatus eq 'Y'}">사용</c:if>	
					    	<c:if test="${list.fbtStatus eq 'N'}">미사용</c:if>	
					    	<c:if test="${list.fbtStatus eq 'A'}">상시사용</c:if>	
					    </td>
					    <td class="td-order" style="text-align:center;">
					    	${list.fbtOrder}
					    </td>
					    <td class="td-mng" style="text-align:center;">
					    	<a href="./write.do?seq=${list.fbtSeq}" class="btn btn-warning btn-xs">
					    		수정
					    	</a>
					    	<a href="javascript:del('${list.fbtSeq}');" class="btn btn-danger btn-xs">
					    		삭제
					    	</a>
				    	</td>
				  	</tr>
