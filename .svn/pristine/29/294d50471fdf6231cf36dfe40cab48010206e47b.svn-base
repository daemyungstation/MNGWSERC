...
<script type="text/javascript" src="/common/js/mngwserc/cn/cna/CNALimitedChoi.js"></script>
<!-- 
	######################################################################
	파일명 		:	CNAZeroChoiList.jsp
	프로그램 명 : 	리미티드 초이스를 조회한다.
	설명		: 	리미티드 초이스를 조회하는 페이지
	작성자		: 	박주윤
	작성일		:	2018.03.29
	수정일자	 			수정자				수정내용
	=====================================================================
...
<form name="frm" method="post" action="${pageLink}">
	...
	<input type="hidden" id="oprtYnChk" value="${rtnMap.list[0].oprtYnChk }">	<!-- 1 보다 크면 Y가 1개 이상임 -->
			...
			<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
				<tr>
					<td style="text-align:center;">
						<input type="checkbox" name="delSeq" value="${list.limtSeq}" />
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectLimitedChoiDtl('${list.limtSeq}');">
						${list.totCnt - (rtnMap.pageIndex - 1) * rtnMap.paginationInfo.pageSize - status.count + 1}
					</td>		
					<td style="text-align:left; cursor:pointer;" onclick="selectLimitedChoiDtl('${list.limtSeq}');">
						${egov:convertDate(list.startDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy-MM-dd', '')}
						~
						${egov:convertDate(list.endDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy-MM-dd', '')}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectLimitedChoiDtl('${list.limtSeq}');">
						${list.title}
					</td>
  	...
  	<div class="modal-dialog">
    	<div class="modal-content">
      		...
      		<div class="modal-body" style="text-align:center;">
				<%@ include file="./CNALimitedChoiPop.jsp" %>
			</div>
    	</div>
  	</div>
</div>
