...
<!-- 
	######################################################################
	파일명 		:	CSACustVoiceWrite.jsp
	프로그램 명 : 	고객의 소리 접수 상세를 조회한다.
	설명		: 	고객의 소리 접수 상세를 조회하는 페이지
	작성자		: 	허진영
	작성일		:	2016.02.07	
	수정일자				수정자				수정내용
	=====================================================================
...
<form name="frm" method="post" action="" >
	...
	<table class="table table-bordered">
		<caption style="display: none;">고객의 소리 접수 관리</caption>
		<colgroup>
			<col width="33%" />
			<col width="34%" />
			<col width="33%" />	
		...
		<tbody>
			<tr>
				<td colspan="3" style="background-color:#f9f9f9">
					<font style="display:inline-block; float:left; margin-left:10px;">
						<b>[${custVoiceInfo.inqryNm} - ${custVoiceInfo.inqryDtlNm}] ${custVoiceInfo.titl}</b>
					</font>
			...
			<tr>
				<td style="padding-left:15px; background-color:#f9f9f9">
					<b>고유번호</b> : ${custVoiceInfo.unqNo}
				</td>
				<td style="padding-left:15px; background-color:#f9f9f9">
					<b>회원명</b> : ${egov:nvl(custVoiceInfo.name,custVoiceInfo.oldRegNm)}
				</td>
				<td style="padding-left:15px; background-color:#f9f9f9">
					<b>작성일</b> : ${egov:convertDate(custVoiceInfo.regDtm, 'yyyy-MM-dd HH:mm', 'yyyy-MM-dd HH:mm', '')}
				</td>
			</tr>
			<tr>
				<td style="padding-left:15px; background-color:#f9f9f9">
					<b>연락처</b> : ${egov:nvl(custVoiceInfo.htel, custVoiceInfo.oldRegCtel)}
				</td>
				<td style="padding-left:15px; background-color:#f9f9f9">
					<b>이메일</b> : ${egov:nvl(custVoiceInfo.email, custVoiceInfo.oldRegEmail)} 
				</td>
				<td style="padding-left:15px; background-color:#f9f9f9">
	...
	<div style="text-align:right; margin-bottom:20px;">
		<a href="javascript:deleteCustVoice();" class="btn btn-danger">삭제</a>
		<a href="javascript:document.listFrm.submit();" class="btn btn-default">취소</a>						
	</div>
	
	<table class="table table-bordered">
		<caption style="display:none;">고객의 소리 접수 답변</caption>
		<colgroup>
			<col width="100%" />
		</colgroup>
		<tbody>
			...
			<tr>
				<td>
					<textarea name="answ" style="height:200px; width:98%;" <c:if test="${custVoiceInfo.mailSendCnt gt 0}">readonly</c:if>>${custVoiceInfo.answ}</textarea>
				</td>
			</tr>
		</tbody>
	</table>
...
<div style="float:right; margin-bottom:20px;">
	<a href="javascript:updateCustVoiceAnsw();" class="btn btn-success">답변저장</a>
...
<table class="table table-bordered" style="margin-top:10px;">
	...
	<tbody>
		<tr>
			<th>확인자</th>
			<td>${rtnMap.custVoiceLog[0].modNm}</td>
			<td>${egov:convertDate(rtnMap.custVoiceLog[0].modDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy-MM-dd HH:mm', '')}</td>
		</tr>
		<tr>
			<th>답변자</th>
			<td>${rtnMap.custVoiceLog[1].modNm}</td>
			<td>${egov:convertDate(rtnMap.custVoiceLog[1].modDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy-MM-dd HH:mm', '')}</td>
		</tr>	
		<tr>
			<th>메일발송</th>
			<td>${rtnMap.custVoiceLog[2].modNm}</td>
			<td>${egov:convertDate(rtnMap.custVoiceLog[2].modDtm, 'yyyy-MM-dd HH:mm:ss', 'yyyy-MM-dd HH:mm', '')}</td>
		</tr>	
	</tbody>
</table>
...
<script type="text/javascript">	
	//고객의 소리 접수 답변저장
	function updateCustVoiceAnsw()
	{
		<c:choose>
			<c:when test="${custVoiceInfo.mailSendCnt lt 1}">
				if(confirm("저장하시겠습니까?"))
				{
					var f = document.frm;
					
					f.action = "./update.do";
					f.custVoiceSeq.value = "${custVoiceInfo.custVoiceSeq}";
					f.submit();		 
				}
			</c:when>
			<c:otherwise>
	...
	function deleteCustVoice()
	{
		if(confirm("삭제하시겠습니까?"))
		{
			...
			f.action = "./delete.do";
			f.delSeq.value = "${custVoiceInfo.custVoiceSeq}";
			f.submit();
		}
	}
	...
	{
		<c:choose>
			<c:when test="${custVoiceInfo.mailSendCnt gt 0}">
				alert("이미 답변 메일발송을 하였습니다.");
			</c:when>
			<c:when test="${custVoiceInfo.prcsCd eq '03'}">
