...
<h6 style="float:left;">대명아임레디 박람회 - 제품 결합상품 관리 : 총 ${rtnMap.list[0].totCnt}건</h6>
...
<form name="frm" action ="${pageLink}">
	...
	<table class="table table-bordered table-hover sorttable">
		<caption style="display: none;">대명아임레디 박람회 - 제품 결합상품 관리</caption>
	 	<thead>
	  		<tr>
			    <th class="td-no">번호</th>
			    <th class="td-title">브랜드</th>
			    <th class="td-title">상품명</th>
			    <th class="td-title">모델명</th>
	...
	<ul id="list_sort">
		<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
	  		<li class="ui-state-default" data-seq="${ list.fbSeq }" data-order="${ list.fbOrder }">
     			<table class="list_sort_table table table-bordered sorttable">
     				<tr>
					    <td class="td-no" style="text-align:center;">
					   	...
					   	<td class="td-title">${ list.fbTitle }</td>
					   	<td class="td-title">${ list.fbSubtitle }</td>
					   	<td class="td-title">${ list.fbModel }</td>
					    <td class="td-auto" style="text-align:left;">
					    	<c:if test="${ list.fbImageWSaveNm ne null }">
					    	<input type="button" value="PC-[${ list.fbImageWRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fbImageWPath }" data-save="${ list.fbImageWSaveNm }"/>
					    	</c:if>
					    	<c:if test="${ list.fbImageMSaveNm ne null }">
					    	<input type="button" value="MO-[${ list.fbImageMRealNm }]" class="btn btn-default btn-xs btn_pop_preview_img" data-path="${ list.fbImageMPath }" data-save="${ list.fbImageMSaveNm }"/>
					    	</c:if>
					    </td>
					    <td class="td-time" style="text-align:center;">
					    	${ list.fbStime } ~ ${ list.fbEtime }
					    </td>
					    <td class="td-use" style="text-align:center;">
					    	<c:if test="${list.fbStatus eq 'Y'}">사용</c:if>	
					    	<c:if test="${list.fbStatus eq 'N'}">미사용</c:if>	
					    	<c:if test="${list.fbStatus eq 'A'}">상시사용</c:if>	
					    </td>
					    <td class="td-order" style="text-align:center;">
					    	${list.fbOrder}
					    </td>
					    <td class="td-mng" style="text-align:center;">
					    	<a class="btn btn-success btn-xs" onclick="javascript:benefitCopy(${list.fbSeq});">
					    		복사
					    	</a>
					    	<a href="./write.do?seq=${list.fbSeq}" class="btn btn-warning btn-xs">
					    		수정
					    	</a>
					    	<a href="javascript:del('${list.fbSeq}');" class="btn btn-danger btn-xs">
					    		삭제
					    	</a>
				    	</td>
				  	</tr>
