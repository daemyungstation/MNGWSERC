...
<!--   
	######################################################################
	파일명 		:	CSAPrsnCnslList.jsp
	프로그램 명 : 	1:1 상담 목록을 조회한다.
	설명		: 	1:1 상담 목록을 조회하는 페이지
	작성자		: 	허진영
	작성일		:	2016.02.07	
	수정일자				수정자				수정내용
	=====================================================================
			...
			<c:forEach var="list" items="${rtnMap.list}" varStatus="status">
	 			<tr>
					<td style="text-align:center;">
						<input type="checkbox" name="delSeq" value="${list.prsnCnslSeq}" />
					</td>
					<td style="text-align:center; cursor:pointer;" onclick="selectPrsnCnslDtl('${list.prsnCnslSeq}');">
						${list.totCnt - (rtnMap.pageIndex - 1) * rtnMap.paginationInfo.pageSize - status.count + 1}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectPrsnCnslDtl('${list.prsnCnslSeq}');">
						${list.inqryNm}
					</td>
					<td style="text-align:left; cursor:pointer;" onclick="selectPrsnCnslDtl('${list.prsnCnslSeq}');">
						${list.inqryDtlNm}
...
<div style="float:left;">
	<a href="javascript:deletePrsnCnslList();" class="btn btn-danger">삭제</a>
</div>

<div style="float:right;">
	<a href="javascript:selectPrsnCnslExcelList();" class="btn btn-info">엑셀 다운로드</a>
</div>
...
<script type="text/javascript">
	...
	//엑셀 다운로드
	function selectPrsnCnslExcelList()
	{
		var f = document.frm;
