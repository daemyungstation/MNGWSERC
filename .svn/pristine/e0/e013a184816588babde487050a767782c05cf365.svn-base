	...
	<div class="well well-small">
		...
		<div class="input-append" style="margin-bottom:0px;">
			<c:when test="${orderdateMng eq 'Y'}">
				발주관리 아이디<br>
				<input type="checkbox" class="id_list_all" id="idListAll" name="idListAll" value="">전체</input><br>
				<c:forEach var="list" items="${rtnMap.admList}">
					<input type="checkbox" class="'id_list"  name="idList" value="${list.id}" }">checked</c:if>>${list.id}</input>
				<%--				<input type="checkbox" class="'id_list"  name="idList" value="${list.id}" <c:if test="${not empty rtnMap.idList && fn:contains(rtnMap.idList, list.id)}">checked</c:if>>${list.id}</input>--%>
			</c:forEach>
			<br>
